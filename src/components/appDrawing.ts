...
export default class appDrawing extends ComponentBase {
  ...
  public constructor(public drawService: DrawService, public window: ng.IWindowService) {
    super();
    this.drawService.getPathsSubject().subscribe((newPaths: Array<PathModel>)=>{
  ...
  protected drawingBranch: Array<PathModel> = [];
  ...
  public keyPressOntextArea(event){
    if(event.key == "Enter"){
      this.textRows += 1;
    }else{
  ...
  private maxLength(array: Array<string>): number{
    var maxL = 20;
    array.forEach((el)=> {
      if(el.length > maxL){
        maxL = el.length;
