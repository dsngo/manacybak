...
export class PathModel{
  constructor(
    public points: string = "", public strokeWidth: string = "",
    public stroke: string = "", public fill: string = "none",
    public fontSize: number = 20, public isText: boolean = false,
    public textPoint: PointModel = new PointModel(),
    public textValue: Array<string> = [],
  ...
  public getSVGElement(): string{
    var elm: string = "";
    if(this.isText){
      elm = `<text x="${this.textPoint.x}" y="${this.textPoint.y}" fill="${this.stroke}" font-size="${this.fontSize}" stroke="${this.textBold}" stroke-width="1px">`;
      for (var index = 0; index < this.textValue.length; index++) {
        var element =this.textValue[index];
        elm += `<tspan x="${this.textPoint.x}" y="${this.textPoint.y + index*this.fontSize}" >${element}</tspan>`;
      }
      elm += "</text>";
    }else{
      elm = `<path stroke-linecap="round" d="${this.points}" fill="none" stroke="${this.stroke}" stroke-width="${this.strokeWidth}"></path>`;
    }
    return elm;
  }
  public static parseString(data: Models.Dtos.SvgElementDto): PathModel{
    ...
    if (doc.documentElement.localName == "path"){
      ...
      for (var index = 0; index < doc.documentElement.childElementCount; index++) {
        var element = doc.documentElement.children[index];
