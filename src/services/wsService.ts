...
export default class WsService extends ServiceBase {
     ...
     * 登録サービス名
     */
    public static readonly IID = "wsService";
    ...
    public load(wsId: string): ng.IPromise<WsService> {
        ...
        const promise4 = this.Restangular
            .one("ws", wsId).all("groups")
        ...
        const promise5 = this.Restangular
            .one("ws", wsId).all("users")
            .getList<Models.Dtos.UserDto>()
            .then((users) => {
                this.userService.loadUsers(users);
                return;
            });

        return this.$q.all([promise1, promise2, promise3, promise4, promise5]).then(() => {
            thisService.ws = resultWs;
            thisService.elementArray = resultElements;
            thisService.entryArray = resultEntries;
            thisService.groupMemberArray = resultMembers;
    ...
    private InitiallizeData(): void {
        ...
        // グループごとのユーザー一覧
        this.groups = _.groupBy(this.groupMemberArray, "groupName");
        ...
        // 現在ログインユーザーが所属するグループ
        this.myGroups = _.pickBy(this.groups, (value: Models.Dtos.WsGroupMemberDto[]) => {
    ...
    /**
     * Entryをサーバーに送信し、保存する
     * entryをpostし、保存されたらその結果をwsServiceのEntry配列にも追加する。
     * @param element 対象element
     ...
     * バイナリー情報リストを読み込む。
     * @param wsId {number}
     * @return {IPromise<WsService>}
     */
    public loadBlobDataList(wsId: number): ng.IPromise<WsService> {
        this.busy(true);
        return this.Restangular
